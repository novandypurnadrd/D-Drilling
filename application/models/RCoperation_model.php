<?php
	...
	class RCoperation_model extends CI_Model {
    ...
    function InputDownhole($data){
      ...
      function InputDetailActivity($data){
		...
		function ViewManpower($start,$end,$site,$location,$rig){
			$a="'";

			$view = $this->db->query('SELECT m.id, m.date, s.name as site, m.shift, l.name as location, r.name as rig, m.position, m.name, m.hoursfrom, m.hoursto, m.comment FROM site s, rig r, location l, manpower m WHERE (m.date) BETWEEN ('.$a.$start.$a.') AND ('.$a.$end.$a.') AND m.site=('.$a.$site.$a.') AND m.location=('.$a.$location.$a.') AND m.rig = ('.$a.$rig.$a.') AND s.id = m.site AND r.id = m.rig AND l.id = m.location');
	    	return $view->result();
			}

			function ViewConsumable($start,$end,$site,$location,$rig){
			$a="'";

			$view = $this->db->query('SELECT c.id, c.date, c.consumable, c.comment, c.quantity, s.name as site, l.name as location, r.name as rig FROM site s, rig r, location l, consumable c WHERE (c.date) BETWEEN ('.$a.$start.$a.') AND ('.$a.$end.$a.') AND c.site=('.$a.$site.$a.') AND c.location =('.$a.$location.$a.') AND c.rig = ('.$a.$rig.$a.') AND s.id = c.site AND r.id = c.rig AND l.id = c.location');
	    	return $view->result();
			}

			function ViewActivity($start,$end,$site,$location,$rig){
			$a="'";

			$view = $this->db->query('SELECT a.id, a.date, ac.activity, sa.subactivity, ssa.subsubactivity, a.comment, a.hours, s.name as site, l.name as location, r.name as rig FROM site s, rig r, location l, detailactivity a, activity ac, subactivity sa, subsubactivity ssa WHERE (a.date) BETWEEN ('.$a.$start.$a.') AND ('.$a.$end.$a.') AND a.site=('.$a.$site.$a.') AND a.location =('.$a.$location.$a.') AND a.rig = ('.$a.$rig.$a.') AND s.id = a.site AND r.id = a.rig AND l.id = a.location AND ac.id = a.activity AND sa.id = a.subactivity AND ssa.id = a.subsubactivity' );
	    	return $view->result();
			}

			function ViewDownhole($start,$end,$site,$location,$rig){
			$a="'";

			$view = $this->db->query('SELECT d.id, d.date, d.description, d.type, d.series, d.quantity, d.comment, s.name as site, l.name as location, r.name as rig FROM site s, rig r, location l, downhole d WHERE (d.date) BETWEEN ('.$a.$start.$a.') AND ('.$a.$end.$a.') AND d.site=('.$a.$site.$a.') AND d.location =('.$a.$location.$a.') AND d.rig = ('.$a.$rig.$a.') AND s.id = d.site AND r.id = d.rig AND l.id = d.location');
	    	return $view->result();
			}

			function ViewSurvey($start,$end,$site,$location,$rig){
			$a="'";

			$view = $this->db->query('SELECT su.id, su.date, su.hole, su.depth, su.dip, su.azimuth, s.name as site, l.name as location, r.name as rig FROM site s, rig r, location l, survey su WHERE (su.date) BETWEEN ('.$a.$start.$a.') AND ('.$a.$end.$a.') AND su.site=('.$a.$site.$a.') AND su.location =('.$a.$location.$a.') AND su.rig = ('.$a.$rig.$a.') AND s.id = su.site AND r.id = su.rig AND l.id = su.location');
	    	return $view->result();
			}
		
			function ViewDrilling($start,$end,$site,$location,$rig){
			$a="'";
